...
KEY_VOLUMEUP    1      /usr/bin/mpc volume +2

# Зменшення гучності на 2% при прокручуванні ліворуч
KEY_VOLUMEDOWN  1      /usr/bin/mpc volume -2
...
Система керування живленням побудована на **ATtiny13** та P-канальному MOSFET. Контролер реалізує автоматичне ввімкнення Raspberry Pi 4, контроль завершення завантаження, ініціацію безпечного вимкнення та фізичне знеструмлення Raspberry Pi після завершення роботи ОС.

Детальна електрична схема, номінали компонентів, фізичні номери контактів Raspberry Pi та підключення MOSFET, ATtiny13 і периферійних компонентів наведені у **[Power-control-schematic](moode_audio_hub.pdf)**.
...
### 1. Модуль керування живленням
[Модуль керування живленням на ATtiny13 та MOSFET](images/power-board-perfboard.jpg)
*Модуль живлення на макетній платі. Той випадок, коли довелося відмовитися від улюблених реле на користь MOSFET (NDP6020P).*

### 2. Внутрішнє компанування пристрою
[Внутрішнє компонування плеєра в зборі](images/internal-assembly_v2.jpg)
*Система Moode Audio в зборі. Raspberry Pi 4 під пасивним охолодженням, PiFi DAC підключений шлейфом, а модуль керування живленням компактно став поруч. У нижній кришці корпусу (піддоні) вирізано отвір для радіатора Armor Case для Raspberry Pi 4. Через цей отвір нижня частина цього радіатора контактує безпосередньо із зовнішнім повітрям.*

[Підключення аудіо-модуля PiFi DAC+](images/dac-connection-close-up.jpg)
*Підключення плати ЦАП (PiFi DAC+ v2.0). Сигнальні лінії - кручені пари МГТФ до зовнішнього аудіо роз'єму.*

### 3. Інтерфейс та передня панель
[Органи керування зі зворотної сторони](images/front-panel-inside.jpg)
*Зворотна сторона передньої панелі. Монтаж та підключення елементів інтерфейсу: механічний тумблер живлення, світлодіод індикації стану та енкодер для керування Moode Audio.*

[Зовнішній вигляд готового плеєра](images/front-panel-outside.jpg)

### Особливості монтажу та пасивного охолодження

* **Радіатор Armor Case:** двосторонній масивний алюмінієвий радіатор. Нижній піддон корпусу має точний виріз, крізь який нижня частина радіатора виходить назовні, забезпечуючи пряме відведення тепла за межі корпусу.
* **Механічне кріплення:** Конструкція стягнута по діагоналях:
  * *Перша діагональ:* штатні гвинти радіатора надійно притискають термопрокладки до відповідних чипів.
  * *Друга діагональ:* кріплення до верхньої частини радіатора плати, нижньої частини радіатора та піддону корпусу. Вирівнюючі шайби компенсують зазор між радіатором та основою корпусу.
* **Кліренс:** корпус встановлено на ніжки висотою 4 мм, що забезпечує достатній зазор для циркуляції повітря між нижньою частиною радіатора та поверхнею, на якій встановлено пристрій.
